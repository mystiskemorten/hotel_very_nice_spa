# Threat Modeling for 'A Very Nice Spa' 
...
The backend of the 'A Very Nice Spa' application, which handles key card accesses, room bookings, and restaurant reservations. 
...
| A vulnerability in a third-party library used by the application is exploited, leading to unauthorized access to sensitive data | We are lacking proper third-party dependency management | - Data breaches, loss of customer trust, and potentially regulatory fines<br>- Using a long time to figure out of the issue due to little control | See [Jira issue #4141]() |
| A denial-of-service (DoS) attack disrupts access to the application preventing legitimate users from making bookings | We sometimes see spikes in increased traffic and are not sure if the service can take it | This could results in signifact revenue loss, customer dissatisfaction and damage the brand's reputation | See [Jira issue #4242]() | 
